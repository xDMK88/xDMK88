  ...
  >
    <template #before-input="props">
      ...
      >
        ...
        <Transition>
          <div
            class="absolute group-hover:flex right-2 top-2 bg-gray-200 dark:bg-gray-700 rounded-lg items-cetner justify-center py-0.5 px-3"
            :class="{ 'flex': isTaskHoverPopperActive && lastSelectedTaskUid == props.node.id, 'hidden': !isTaskHoverPopperActive }"
            :style="{ backgroundColor: colors[props.node.info.uid_marker] ? colors[props.node.info.uid_marker].back_color : '' }"
          >
            <Icon
              :path="subtask.path"
            ...
            <Popper
              arrow
              @open:popper="toggleTaskHoverPopper"
              @close:popper="toggleTaskHoverPopper"
...
export default {
  ...
  setup () {
    ...
    const toggleTaskHoverPopper = () => {
      isTaskHoverPopperActive.value = !isTaskHoverPopperActive.value
    }

...
<template>
  <li class="item-element-class">
    <div
      :class="{ bold: isFolder }" class="list-tags-access"
      @click="toggle"
      @dblclick="changeType">
...
<path d="M6.87133 3.22989C7.4288 3.59197 7.42879 4.40803 6.87133 4.77011L2.12908 7.8503C1.51816 8.2471 0.71057 7.80866 0.71057 7.08018V0.919817C0.71057 0.191341 1.51816 -0.247102 2.12908 0.149703L6.87133 3.22989Z" fill="black" fill-opacity="0.5"/>
      ...
      <label class="break-words">{{ model.name }}</label>    <input type="checkbox" name="check_tags" class="check-custom-project" v-model="select[selectedTask.tags]" :value="model.uid" @click="changetags(selectedTask.uid, model.uid)" :checked="selectedTask.tags.filter(tag=>tag===model.uid)[0]===model.uid">
    </div>

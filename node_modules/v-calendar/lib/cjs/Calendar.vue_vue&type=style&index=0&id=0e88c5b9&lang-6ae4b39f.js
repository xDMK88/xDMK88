'use strict';var helpers=require('./helpers-1afc4b71.js'),vue=require('vue'),index=require('./index-1621a98b.js'),CalendarPane_vue_vue_type_style_index_0_id_bf16a152_lang=require('./CalendarPane.vue_vue&type=style&index=0&id=bf16a152&lang-8a8157ef.js'),CalendarNav=require('./CalendarNav-0346c78f.js'),CustomTransition=require('./CustomTransition.js'),styleInject_es=require('./style-inject.es-06def3b0.js'),Popover_vue_vue_type_style_index_0_id_5059a25e_lang=require('./Popover.vue_vue&type=style&index=0&id=5059a25e&lang-1a220d5a.js'),PopoverRow=require('./PopoverRow.js'),SvgIcon=require('./SvgIcon.js');/**
 * @name addMonths
...
function addYears(dirtyDate, dirtyAmount) {
    ...
    value: function refresh(attrs, reset) {
      ...
      if (helpers.l(attrs)) {
        attrs.forEach(function (attr, i) {
          ...
          var hashcode = helpers.m(JSON.stringify(attr));
  ...
  helpers.o(element, 'touchstart', touchStart, {
    passive: true
  }); // on(element, 'touchmove', touchmove);

  helpers.o(element, 'touchend', touchEnd, {
    passive: true
  }); // Return function that removes event handlers

  return function () {
    helpers.p(element, 'touchstart', touchStart); // off(element, 'touchmove', touchmove);

    helpers.p(element, 'touchend', touchEnd);
  };
};var script = {
  name: 'Calendar',
  ...
  render: function render() {
    ...
    var panes = this.pages.map(function (page, i) {
      ...
      return vue.h(CalendarPane_vue_vue_type_style_index_0_id_bf16a152_lang.s, helpers.n(helpers.n({}, _this.$attrs), {}, {
        key: page.key,
    ...
    var getNavPopover = function getNavPopover() {
      return vue.h(Popover_vue_vue_type_style_index_0_id_5059a25e_lang.s, {
    ...
    return vue.h('div', {
      ...
      default: function _default() {
        return vue.h('div', helpers.n(helpers.n({}, _this.$attrs), {}, {
  ...
  data: function data() {
    return {
      ...
      inTransition: false,
      sharedState: {
        navPopoverId: helpers.q(),
        dayPopoverId: helpers.q(),
        theme: {},
        masks: {},
        locale: {}
      }
  ...
  methods: {
    ...
    canMove: function canMove(arg) {
      ...
      if (index.h(arg)) position = 1; // Set position if unspecified and out of current bounds

      if (!position) {
        if (helpers.t(page, this.firstPage)) {
          position = -1;
        } else if (helpers.u(page, this.lastPage)) {
          position = 1;
        } else {
      ...
      return helpers.v(opts.fromPage, opts.toPage).some(function (p) {
        return helpers.w(p, _this3.minPage_, _this3.maxPage_);
      });
    },
    ...
    move: function move(arg) {
      ...
      if (opts.fromPage && !helpers.s(opts.fromPage, this.firstPage)) {
        return this.refreshPages(helpers.n(helpers.n({}, opts), {}, {
          page: opts.fromPage,
          position: 1,
          force: true
        }));
    ...
    showPageRange: function showPageRange(range, opts) {
      ...
      } else if (helpers.y(range)) {
        var month = range.month,
            year = range.year;
        var from = range.from,
            to = range.to;

        if (index.h(month) && index.h(year)) {
          fromPage = range;
        } else if (from || to) {
          fromPage = helpers.x(from) ? this.pageForDate(from) : from;
          toPage = helpers.x(to) ? this.pageForDate(to) : to;
        }
      } else {
      ...
      if (helpers.u(toPage, lastPage)) {
        page = helpers.z(toPage, -(this.pages.length - 1));
      } // But no earlier than the desired `fromPage`


      if (helpers.t(page, fromPage)) {
        page = fromPage;
      }
    ...
    getTargetPageRange: function getTargetPageRange(page) {
      ...
      if (helpers.A(page)) {
        ...
        fromPage = helpers.z(page, pagesToAdd);
      } else {
        fromPage = this.getDefaultInitialPage();
      }

      toPage = helpers.z(fromPage, this.count - 1); // Adjust range for min/max if not forced

      if (!force) {
        if (helpers.t(fromPage, this.minPage_)) {
          fromPage = this.minPage_;
        } else if (helpers.u(toPage, this.maxPage_)) {
          fromPage = helpers.z(this.maxPage_, 1 - this.count);
        }

        toPage = helpers.z(fromPage, this.count - 1);
      }
    ...
    getDefaultInitialPage: function getDefaultInitialPage() {
      ...
      if (!helpers.A(page)) {
        ...
        if (helpers.A(toPage)) {
          page = helpers.z(toPage, 1 - this.count);
        }
    ...
    refreshPages: function refreshPages() {
      ...
      return new Promise(function (resolve, reject) {
        ...
        for (var i = 0; i < _this5.count; i++) {
          pages.push(_this5.buildPage(helpers.z(fromPage, i), ignoreCache));
    ...
    getPageTransition: function getPageTransition(oldPage, newPage) {
      ...
      if (transition === 'fade' || !transition && this.count > 1 || !helpers.A(oldPage) || !helpers.A(newPage)) {
        return 'fade';
      } // Moving to a previous page


      var movePrev = helpers.t(newPage, oldPage); // Vertical slide
    ...
    getPageForAttributes: function getPageForAttributes() {
      ...
      if (attr && attr.hasDates) {
        var _attr$dates = helpers.B(attr.dates, 1),
            date = _attr$dates[0];
    ...
    refreshAttrs: function refreshAttrs() {
      ...
      if (!helpers.l(pages)) return; // For each page...
      ...
      pages.forEach(function (p) {
        ...
        p.days.forEach(function (d) {
          ...
          if (reset) {
            shouldRefresh = true;
          } else if (helpers.C(d.attributesMap, deletes)) {
            // Delete attributes from the delete list
            map = helpers.D(d.attributesMap, deletes); // Flag day for refresh
          ...
          adds.forEach(function (attr) {
            ...
            if (targetDate) {
              var newAttr = helpers.n(helpers.n({}, attr), {}, {
        ...
        case 'Home':
          {
            ...
            newDate = index.j(date, -day.weekdayPosition + 1);
            break;
          }
        ...
        case 'End':
          {
            ...
            newDate = index.j(date, day.weekdayPositionFromEnd);
            break;
          }
